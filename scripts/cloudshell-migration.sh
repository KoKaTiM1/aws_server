...
DB_CREDS=$(aws secretsmanager get-secret-value --secret-id eyedar-prod-db-credentials --region us-east-1 --query 'SecretString' --output text)
...
PGPASSWORD="$DB_PASSWORD" psql -h eyedar-prod-db.csfmmaq82w8d.us-east-1.rds.amazonaws.com -U "$DB_USERNAME" -d eyedar -f migration.sql

# 5. Verify migration (optional)
echo "Verifying tables..."
PGPASSWORD="$DB_PASSWORD" psql -h eyedar-prod-db.csfmmaq82w8d.us-east-1.rds.amazonaws.com -U "$DB_USERNAME" -d eyedar -c "\dt"

echo "Verifying PostGIS..."
PGPASSWORD="$DB_PASSWORD" psql -h eyedar-prod-db.csfmmaq82w8d.us-east-1.rds.amazonaws.com -U "$DB_USERNAME" -d eyedar -c "SELECT PostGIS_Version();"
